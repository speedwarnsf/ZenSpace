  ...
  async function regenerateRoom(roomId: string) {
    ...
    try {
      const response = await fetch('/api/listings/regenerate-room', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ listingId, roomId })
      });
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <div className={`px-3 py-1 text-xs font-medium tracking-wide uppercase ${
                listing.status === 'review' ? 'bg-amber-900 text-amber-300' : 'bg-green-900 text-green-300'
              }`}>
                {listing.status}
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
            ...
            return (
              <div key={room.id} className="bg-stone-950 border border-stone-800">
                ...
                <div className="p-6 border-b border-stone-800">
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-2 bg-amber-600 text-stone-900 hover:bg-amber-500 transition-colors"
                          >
                  ...
                  {/* Original Photo */}
                  <div className="mt-4">
                    <div className="text-amber-600 text-xs font-medium tracking-wide mb-2 uppercase">
                      Original
                    </div>
                    <img
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {room.designs.map(design => {
                        ...
                        return (
                          <div
                            key={design.id}
                            className={`border ${state.approved ? 'border-amber-600' : 'border-stone-700'} bg-stone-900`}
                          >
                            {/* Design Image */}
                            <div className="relative">
                              ...
                              <button
                                ...
                                className={`absolute top-2 right-2 p-2 ${
                                  state.isHero ? 'bg-amber-600 text-stone-900' : 'bg-stone-900/80 text-stone-400'
                                } hover:bg-amber-600 hover:text-stone-900 transition-colors`}
        ...
        <div className="mt-12 flex justify-end">
          <button
            ...
            className="px-8 py-4 bg-amber-600 text-stone-900 font-bold text-lg hover:bg-amber-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-3"
          >
            {publishing ? (
              <>
