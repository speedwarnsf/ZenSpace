...
// ═══════════════════════════════════════════════════════════════════
//  NUDIO LABS PORTRAIT ENGINE — all options from labsLighting.js
// ═══════════════════════════════════════════════════════════════════
...
const LABS_WARDROBE_FEMALE = [
  ...
  },
  {
    ...
    label: 'Vintage chic',
    prompt: 'vintage-inspired luxury drawing from Chanel couture, Dior New Look, and Gucci archives—elegant midi silhouettes, nipped waists, silk scarves, brooches, and refined glove details',
    useOriginal: false,
  },
  {
    id: 'style-women-sport',
    label: 'Sport luxe',
    prompt: 'elevated performance wear inspired by Prada Linea Rossa, Moncler Grenoble, and Louis Vuitton sport capsules—sleek technical fabrics, structured windbreakers, bold stripes, and sculpted sneakers',
    useOriginal: false,
  },
];

// ─── Backdrop Variants — ALL 15 Savage Seamless ───

const BACKDROP_VARIANTS = [
  { id: 'pink', label: 'Pink', description: 'Savage Seamless Light Pink (#08, hex #F6CADC)', tone: 'the signature nudio pink backdrop' },
...
// ─── Lighting Setups — ALL 46 from LABS_LIGHTING_GROUPS ───
...
function pickUniqueN<T extends { id: string }>(arr: T[], n: number): T[] {
  const shuffled = [...arr];
...
function generateCombos(gender: string): PortraitCombo[] {
  ...
  // For pools with >= 3 items, pick 3 unique. For pools with < 3, allow reuse.
  const lenses = pickUniqueN(LABS_LENS_CHOICES, 3);
  const retouches = pickUniqueN(LABS_RETOUCH_CHOICES, 3);
  const wardrobes = pickUniqueN(wardrobePool, 3);
  const backdrops = pickUniqueN(BACKDROP_VARIANTS, 3);
  const lightings = pickUniqueN(LIGHTING_SETUPS, 3);
